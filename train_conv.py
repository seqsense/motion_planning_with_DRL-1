...
ENV = 'myenv-v5'
env = gym.make(ENV)
NUM_LIDARS = env.NUM_LIDAR
NUM_GOALS = env.NUM_TARGET
NUM_ACTIONS = env.action_space.shape[0]
A_BOUNDS = [env.action_space.low, env.action_space.high]
NONE_STATE_L = np.zeros(NUM_LIDARS)
NONE_STATE_G = np.zeros(NUM_GOALS)
...
class PPONet(object):
    def __init__(self,sess):
        ...
        self.s_l = tf.placeholder(tf.float32, shape=(None, NUM_LIDARS, 1))
        self.s_g = tf.placeholder(tf.float32, shape=(None, NUM_GOALS))
    ...
    def _build_net(self,name,trainable):
        with tf.variable_scope(name):
            conv1 = tf.layers.conv1d(inputs=self.s_l, filters=8, kernel_size=5, strides=2, padding='SAME',activation=tf.nn.relu,trainable=trainable)
            conv2 = tf.layers.conv1d(inputs=conv1, filters=8, kernel_size=3, strides=2, padding='SAME',activation=tf.nn.relu,trainable=trainable)
            mb, h, c = conv2.get_shape().as_list()
            feature_shape = h * c
            feature_flat = tf.reshape(conv2, [-1,feature_shape])
            fc1 = tf.layers.dense(feature_flat, 64, tf.nn.relu,trainable=trainable)
            fc2 = tf.layers.dense(fc1, 16, tf.nn.relu,trainable=trainable)
            s_t = tf.concat([fc2,self.s_g],1)
            l1 = tf.layers.dense(s_t, NUM_HIDDENS[0],tf.nn.relu,trainable=trainable)
    ...
    def update_parameter_server(self):     # 重みを学習・更新します
        ...
        for i in range(EPOCH):
            ...
            for n in range(n_batches):
                sl, sg, a, r, sl_, sg_, s_mask = np.array(batch[n * MIN_BATCH: (n + 1) * MIN_BATCH]).T
                sl = np.vstack(sl)
                sg = np.vstack(sg)
                a = np.vstack(a)
                r = np.vstack(r)
                ...
                sl = np.reshape(sl, [MIN_BATCH, NUM_LIDARS, 1])
                # 時間割引総報酬vを求めます
                v = self.sess.run(self.v, feed_dict={self.s_l:sl, self.s_g: sg})

                # N-1ステップあとまでの時間割引総報酬rに、Nから先に得られるであろう総報酬vに割引N乗したものを足します
                r = r + LAMBDA * GAMMA_N * v * s_mask  # set v to 0 where s_ is terminal state

                feed_dict = {self.s_l: sl, self.s_g: sg, self.a_t: a, self.r_t: r}
                minimize = self.graph
                self.sess.run(minimize, feed_dict)   # Brainの重みを更新

        summary_str = self.sess.run(summary_ops, feed_dict = {
            summary_vars[0]: r.mean(),
            summary_vars[1]: self.sess.run(self.entropy, feed_dict={self.s_l:sl, self.s_g:sg}) / LOSS_ENTROPY,
            summary_vars[2]: self.sess.run(self.learning_rate),
            summary_vars[3]: self.sess.run(self.loss_CLIP, feed_dict),
            summary_vars[4]: self.sess.run(self.loss_value, feed_dict={self.s_l: sl, self.s_g: sg, self.r_t: r}),
            summary_vars[5]: self.sess.run(self.v, feed_dict={self.s_l: sl, self.s_g: sg}).mean()
        })
        writer.add_summary(summary_str,GLOBAL_EP)
        writer.flush()

    def predict_a(self, sl, sg):    # 状態sから各action
        sl = np.reshape(sl, [1, NUM_LIDARS, 1])
        a = self.sess.run(self.A, feed_dict={self.s_l: sl, self.s_g: sg})
        return a

    def train_push(self, sl, sg, a, r, sl_, sg_):
        self.train_queue[0].append(sl)
        self.train_queue[1].append(sg)
        self.train_queue[2].append(a)
        self.train_queue[3].append(r)

        if sl_ is None:
            self.train_queue[4].append(NONE_STATE_L)
            self.train_queue[5].append(NONE_STATE_G)
            self.train_queue[6].append(0.)
        else:
            self.train_queue[4].append(sl_)
            self.train_queue[5].append(sg_)
...
class Agent:
    ...
    def act(self, sl,sg):
        ...
        if np.random.rand() < eps:
            ...
            sl = np.array([sl])
            sg = np.array([sg])
            a = self.brain.predict_a(sl,sg).reshape(-1)
            return a

    def advantage_push_brain(self, sl, sg, a, r, sl_, sg_):
        def get_sample(memory, n):
            sl, sg, a, _, _, _ = memory[0]
            _, _, _, _, sl_, sg_ = memory[n - 1]
            return sl, sg, a, self.R, sl_, sg_

        self.memory.append((sl, sg, a, r, sl_, sg_))
        ...
        if sl_ is None:
            while len(self.memory) > 0:
                n = len(self.memory)
                self.R = self.R - LAMBDA * self.memory[0][3] + self.memory[0][3]
                sl, sg, a, r, sl_, sg_ = get_sample(self.memory, n)
                self.brain.train_push(sl, sg, a, r, sl_, sg_)
        ...
        if len(self.memory) >= N_STEP_RETURN:
            self.R = self.R - LAMBDA * self.memory[0][3] + self.memory[0][3]
            sl, sg, a, r, sl_, sg_ = get_sample(self.memory, N_STEP_RETURN)
            self.brain.train_push(sl, sg, a, r, sl_, sg_)
            self.R = self.R - self.memory[0][3]     # # r0を引き算
            self.memory.pop(0)
...
class Environment:
    ...
    def run(self):
        ...
        sl, sg = self.env.reset()
        R = 0
        step = 0
        while True:
            #if self.name == 'W_0':
            #    self.env.render()
            a = self.agent.act(sl,sg)
            sl_,sg_, r, done, info = self.env.step(a)
            ...
            # 報酬と経験を、Brainにプッシュ
            self.agent.advantage_push_brain(sl, sg,  a, r, sl_, sg_)
