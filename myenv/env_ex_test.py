...
class MyEnv(gym.Env):
    ...
    def render(self, mode='human', close=False):
        ...
        if self.viewer is None:
            ...
            l = (margin+400*self.MAP_RESOLUTION) * scale
            r = (margin+800*self.MAP_RESOLUTION) * scale
            t = (margin+600*self.MAP_RESOLUTION) * scale
            b = (margin+700*self.MAP_RESOLUTION) * scale
            ob2 = rendering.FilledPolygon([(l,b),(l,t),(r,t),(r,b)])
            ob2.set_color(0.,0.,0.)
            self.viewer.add_geom(ob2)
            
            ob3 = rendering.make_circle((100*self.MAP_RESOLUTION)*scale)
            ob3_trans = rendering.Transform()
            ...
            ob4_trans.set_translation((margin+500*self.MAP_RESOLUTION)*scale,(margin+250*self.MAP_RESOLUTION)*scale)
            ob4.add_attr(ob4_trans)
            ob4.set_color(0.0,0.0,0.0)
            self.viewer.add_geom(ob4)

            ob5 = rendering.make_circle((50*self.MAP_RESOLUTION)*scale)
            ob5_trans = rendering.Transform()
            ob5_trans.set_translation((margin+800*self.MAP_RESOLUTION)*scale,(margin+350*self.MAP_RESOLUTION)*scale)
            ob5.add_attr(ob5_trans)
...
#        reward += 1./(200.*np.pi)*angle_diff(np.arctan2((self.pose[1]-self.target[1]),(self.pose[0]-self.target[0])),self.pose[2])
    ...
    def is_goal(self):
        if self.dis < self.robot_radius:
            self.num_goal += 1
            if self.num_goal <= len(self.target_pose):
                self.target = self.target_pose[self.num_goal-1]
                return False
            else:
                return True
        else:
