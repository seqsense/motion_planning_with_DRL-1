...
class MyEnv(gym.Env):
    ...
    def __init__(self):
        ...
        for i in range(self.NUM_STATES):
            self.observation_low[self.NUM_LIDAR][i] = self.min_distance
            self.observation_low[self.NUM_LIDAR+1][i] = -1.
            self.observation_low[self.NUM_LIDAR+2][i] = -1.
            self.observation_high[self.NUM_LIDAR][i] = self.max_distance
            self.observation_high[self.NUM_LIDAR+1][i] = 1.
            self.observation_high[self.NUM_LIDAR+2][i] = 1.
...
#        reward += 1./(200.*np.pi)*angle_diff(np.arctan2((self.pose[1]-self.target[1]),(self.pose[0]-self.target[0])),self.pose[2])
    ...
    def observe(self):
        observation = np.zeros(self.NUM_LIDAR+self.NUM_TARGET)
        #LIDAR
        for i in range(self.NUM_LIDAR):
            lidar = []
